...
def run_gui_tests():
    ...
    try:
        ...
        def counting_play_click(*args, **kwargs):
            play_click_count[0] += 1
            return orig_play_click(*args, **kwargs)
    ...
    try:
        ...
        def finish_tests():
            root.destroy()
            # After all tests, check click sound count
            if play_click_count[0] == total_button_presses[0]:
                result.passed.append(f"Click sound triggered for every button press: {play_click_count[0]} times.")
            else:
                result.errors.append(f"Click sound triggered {play_click_count[0]} times, but {total_button_presses[0]} button presses occurred.")
